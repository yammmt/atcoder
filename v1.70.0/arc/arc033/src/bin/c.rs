...
#[fastout]
fn main() {
    const A_MAX: usize = 200_002;
    input! {
        q: usize,
        txq: [(usize, usize); q],
    }

    let zeros = vec![0; A_MAX];
    let mut st = SegTree::new(A_MAX, 0, |&a, &b| a + b);
    st.build(&zeros);

    for (t, x) in txq {
        if t == 1 {
            st.update(x, st.fold(x, x + 1) + 1);
        } else {
            // x 番目に小さい = その数より小さい数は x-1 個
            // pass: その数より小さい数は x 個未満
            let mut pass = 0;
            let mut fail = A_MAX;
            while fail - pass > 1 {
                let mid = (pass + fail) / 2;
            ...
            println!("{pass}");
            st.update(pass, st.fold(pass, pass + 1) - 1);
        }
    }
}
