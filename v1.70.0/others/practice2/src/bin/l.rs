...
fn main() {
    input! {
        ...
        q: usize,
        an: [usize; n],
        tlrq: [(usize, Usize1, usize); q],
    }

    // - 値データ: (区間内で a_i = 0 となる要素の個数, 区間内で a_i = 1 となる要素の個数, 区間内の転倒数)
    // - 遅延データ: 区間全体に対して書き換えが行われた回数の偶奇

    // 合成後の転倒数 = それぞれの区間の転倒数 + 左 1 個数 * 右 0 個数
    let combine_node = |a: &(usize, usize, usize), b: &(usize, usize, usize)| {
        (a.0 + b.0, a.1 + b.1, a.2 + b.2 + a.1 * b.0)
    };
    // 反転回数
    let combine_lazy = |a: &usize, b: &usize| *a ^ *b;
    // 転倒数: (1, 0) <-> (0, 1) で減るもののみ
    let reflect = |node: &(usize, usize, usize), lazy: &usize| {
        if *lazy == 0 {
            *node
        } else {
            (node.1, node.0, node.0 * node.1 - node.2)
        }
    };
    let mut lst = LazySegTree::new(n, (0, 0, 0), 0, combine_node, combine_lazy, reflect);
    let mut bn = Vec::with_capacity(n);
    for a in an {
        bn.push(if a == 0 { (1, 0, 0) } else { (0, 1, 0) });
    }
    lst.build(&bn);
