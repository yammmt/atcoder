...
fn main() {
    ...
    // - 値データ: (整数列の値の和, 区間の長さ)
    // - 遅延データ: クエリで与えられる (b, c) の組を管理

    let combine_node = |a: &(usize, usize), b: &(usize, usize)| ((a.0 + b.0) % MOD, a.1 + b.1);
    let combine_lazy =
        |a: &(usize, usize), b: &(usize, usize)| ((a.0 * b.0) % MOD, (a.1 * b.0 + b.1) % MOD);
    let reflect = |a: &(usize, usize), b: &(usize, usize)| {
        // |node, lazy|
        ((b.0 * a.0 + b.1 * a.1) % MOD, a.1)
    };

    let mut lst = LazySegTree::new(n, (0, 0), (1, 0), combine_node, combine_lazy, reflect);
